...
def execute(command, stdin=None, stdout=None, stderr=None):
    answer = ''
...
class GitRebaser(object):
    ...
    def __init__(self, repository_path, primary_branch, secondary_branch):
...
def psykorebase(args=sys.argv[1:]):
    ...
    secondary_branch = parsed_args.secondary_branch[0]
    print "Working with %s, rebasing %s on top of %s" % (dvcs_type,
                                                         secondary_branch,
                                                         primary_branch)
